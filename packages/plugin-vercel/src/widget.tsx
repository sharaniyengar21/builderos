...
const STATE_CLASSES: Record<string, string> = {
  ...
  CANCELED: "bg-white/[0.06] text-ink-muted",
};
...
export function VercelStatsWidget({ connectionId, projectName, initialSnapshot, lastSyncedAt }: VercelStatsWidgetProps) {
  ...
  const stateClass = snapshot.latestDeployment ? STATE_CLASSES[snapshot.latestDeployment.state] ?? STATE_CLASSES.CANCELED : null;

  return (
    <div className="glass-surface rounded-xl p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <Triangle className="h-5 w-5 text-ink-secondary" />
          <div>
            <h3 className="text-sm font-medium text-ink-primary">{projectName}</h3>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <StatTile label="Deployments (24h)" value={snapshot.deploymentsLast24h} />
        <StatTile label="Deployments (7d)" value={snapshot.deploymentsLast7d} />
      </div>

      {snapshot.latestDeployment && (
        <a
          href={`https://${snapshot.latestDeployment.url}`}
          target="_blank"
          rel="noreferrer"
          className={`mt-3 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs ${stateClass}`}
        >
          {snapshot.latestDeployment.state}
